...
CampaignOS adds a Shareable Client Portal — a sister surface to the Client-Ready Report tuned for client-facing handoff. `buildClientPortalSnapshot({ project, runs, reviewSummary?, agencySelection?, learningMemory?, results?, accuracyReport?, visibility? })` is pure / deterministic: it reads stored workspace, review, results, agency, and learning data and emits a `ClientPortalSnapshot` with twelve canonical sections in stable order — `overview` → `strategy-snapshot` → `offer` → `audience` → `proof` → `execution` → `forecast` → `simulator` → `benchmarks` → `review-status` → `results` → `next-actions`. Sections whose visibility is off OR whose derived content is empty are omitted; the renderer never emits empty headers. `generatedAt` is the max of every stored `updatedAt`/`runAt`/`derivedAt` value the caller passes — never `Date.now()` — so two builds against identical state are byte-identical. `renderClientPortalMarkdown(snapshot)` emits a printable markdown file; `buildPortalSharePack(snapshot, markdown)` returns a one-liner suitable for the "Copy client summary" button. Three high-level toggles — `hidePricing`, `hideInternalNotes` (default on), `hideAssumptions` — control the founder-facing scrubbing posture: `hidePricing` strips `$` / price labels from the Offer section; `hideAssumptions` drops the Forecast and Simulator sections entirely; `hideInternalNotes` suppresses the internal-reference footer line. The deliverable lives at the dedicated `/portal` route: a single-column, print-friendly page that lazy-creates browser stores in `useEffect`, exposes per-section checkboxes plus the three high-level toggles, surfaces a project picker when multiple projects exist, and ships a Print / Save as PDF button, a Download Markdown button (blob download named `campaignos-portal-<slug>.md`), and a Copy client summary button. The export brief gains an optional `## Client Portal Pack` section (last-published date, overview headline, approval status line, top next actions) when the caller passes the portal context and the snapshot has at least one visible section; absent or empty → byte-identical to the pre-portal build. The portal SITS ABOVE the engine: `buildStrategy(input)` stays byte-identical; the portal is a derived view over stored data, not a new engine module.
...
│   │   ├── report/page.tsx Client-Ready Report — `/report` route, print-friendly, deterministic
│   │   └── portal/page.tsx Shareable Client Portal — `/portal` route, print-friendly, deterministic, 12 sections + scrub toggles
│   ├── components/
...
│   │   ├── portal/             Shareable Client Portal (above the engine — derived view over stored data)
│   │   │   ├── portal-builder.ts       buildClientPortalSnapshot() / buildPortalSharePack() / defaultPortalVisibility() — pure deterministic derivation, generatedAt never reads Date.now()
│   │   │   └── portal-markdown.ts      renderClientPortalMarkdown() — printable markdown export; omits empty sections, scrubs based on visibility toggles
