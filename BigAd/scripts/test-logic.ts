...
// ---- CampaignOS naming polish — brand-lock reinforcement -------------------
//
// Step 3 lock-in: every user-facing exporter that shipped after the soft
// rebrand (commit 20b786c) must surface "CampaignOS" and must not leak the
// retired BigAd brand outside the single allow-listed internal-reference
// footer in the portal (and only when hideInternalNotes=false).
//
// These asserts re-use the AstroDating example and the deterministic
// engine output `a` already built at the top of this file.

{
  // Export brief — CampaignOS only on user-facing surfaces.
  const lockBrief = generateExportBrief(ASTRO_DATING_EXAMPLE, a);
  record(
    "naming-polish: export brief contains 'CampaignOS' (post-rebrand surfaces)",
    lockBrief.includes("CampaignOS")
  );
  record(
    "naming-polish: export brief never says 'Generated by BigAd'",
    !lockBrief.includes("Generated by BigAd")
  );
  record(
    "naming-polish: export brief never says 'BigAd suggests:'",
    !lockBrief.includes("BigAd suggests:")
  );
  record(
    "naming-polish: export brief never says '(formerly BigAd)'",
    !lockBrief.includes("(formerly BigAd)")
  );

  // Report markdown — CampaignOS only, no parenthetical BigAd, no old header.
  const { renderClientReportMarkdown: renderReportLock } = require("../src/lib/report/report-markdown");
  const { buildClientReport: buildReportLock } = require("../src/lib/report/report-builder");
  const LOCK_TIME = "2026-05-17T12:00:00.000Z";
  const lockProject = {
    metadata: {
      id: "naming-lock-proj",
      name: "AstroDating Naming-Lock",
      createdAt: LOCK_TIME,
      updatedAt: LOCK_TIME,
      runCount: 1,
    },
  ...
  const lockRun = {
    id: "naming-lock-run",
    projectId: "naming-lock-proj",
    runAt: LOCK_TIME,
    input: ASTRO_DATING_EXAMPLE,
    strategy: a,
  };
  const lockReport = buildReportLock({
    project: lockProject,
    runs: [lockRun],
    testResults: [],
    learningMemory: {
    ...
    generatedAt: LOCK_TIME,
  });
  const lockReportMd = renderReportLock(lockReport);
  record(
    "naming-polish: report markdown title says CampaignOS",
    lockReportMd.includes("# CampaignOS Report")
  );
  record(
    "naming-polish: report markdown never says '(formerly BigAd)'",
    !lockReportMd.includes("(formerly BigAd)")
  );
  record(
    "naming-polish: report markdown never leaks bare 'BigAd' brand token",
    !lockReportMd.includes("BigAd")
  );

  // Portal markdown — CampaignOS only on user-facing body; the internal-
  // reference footer is hidden by default and is the ONLY allow-listed
  // surface that may carry the legacy folder name (and even there only
  // when the operator explicitly opts in by clearing hideInternalNotes).
  const {
    buildClientPortalSnapshot: buildPortalLock,
    defaultPortalVisibility: defaultVisLock,
  } = require("../src/lib/portal/portal-builder");
  const { renderClientPortalMarkdown: renderPortalLock } = require("../src/lib/portal/portal-markdown");
  const lockSnap = buildPortalLock({
    project: lockProject,
    runs: [lockRun],
    visibility: defaultVisLock(),
  });
  const lockPortalMd = renderPortalLock(lockSnap);
  record(
    "naming-polish: portal markdown title says CampaignOS",
    lockPortalMd.includes("# CampaignOS Client Portal")
  );
  record(
    "naming-polish: portal markdown (default visibility) never contains 'BigAd'",
    !lockPortalMd.includes("BigAd")
  );
}
