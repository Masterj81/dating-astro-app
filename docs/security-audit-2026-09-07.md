...
| **JUNO-05** | PWA / Web | Jetons de session en `localStorage` + CSP `script-src 'unsafe-inline'` | **Haute** | Haute | Confirmé — **PARTIELLEMENT CORRIGÉ le 21 sep 2026 — WEB STORAGE SUPPRIMÉ, fermeture conditionnée à l'enforcement CSP en production** (worktree, preuve Production requise) : reproduction code→bibliothèque (createClient sans adaptateur → `globalThis.localStorage`, 37 appelants) ; migration `@supabase/ssr@0.12.7` + supabase-js 2.114.0 (minimum peer, audit delta nul) via le point unique `getSupabaseBrowser` (zéro appelant modifié) ; cookies `__Host-juno.sb` prod (Secure, SameSite=Lax, Path=/, chunkés) ; purge contrôlée des clés legacy (aucun jeton lu/copié) ; refresh middleware officiel ; API restent Bearer + garde Origin fail-closed prête ; test TRANCHANT jsdom + validateur CI à canaris. **Limite assumée : cookies NON HttpOnly — les jetons restent lisibles par JavaScript ; tant que la CSP sans `'unsafe-inline'` (JUNO-13) est en Report-Only, l'extraction XSS n'est pas bloquée** ; le vrai HttpOnly = BFF, chantier séparé — voir runbook web-session-csp |
| **JUNO-06** | Mobile | 9 fonctionnalités premium sur 11 gatées côté client uniquement | **Haute** | Haute | Confirmé — **PARTIELLEMENT CORRIGÉ le 23 sep 2026 (reprise) — CONTRÔLES SERVEUR RÉELS SUR 2/11, 9/11 RESTENT CONTOURNABLES DANS L'APK ; migration, déploiement des edges et preuve production requis**. La v1 (22 sep) routait 11/11 par `enforce_premium_feature` ; rejetée comme correction de sécurité au motif qu'un appel serveur préalable n'est pas une autorisation lorsque le résultat premium reste produisible hors ligne. Reprise : (1) **tarot = artefact serveur** — edge `premium-tarot-reading` (la décision enforce est DANS l'edge, 402 sans octets, 503 fail-closed ; `generateReading` avec l'artefact committé `tarot.generated.ts`, généré par `build-edge-tarot` comme le bundle astrologie) et **zéro import `@astro/shared/tarot` dans le mobile** ; (2) **flux de sync serveur-vérifié** — edge `sync-entitlement` (le serveur interroge RevenueCat avec SES credentials, écrit `subscriptions`, n'accorde jamais) et purge du lissage local dans PremiumGate/conversation-guide/PremiumContext (après un refus : état `sync_available` + « Vérifier mon abonnement », seul le nouveau verdict enforce accorde) ; (3) **inventaire honnête 3 classes** — `enforcement_class` en base (migration auto-vérifiée, compteurs 2/7/2), `ENFORCEMENT_CLASSES` client, validateurs à compteurs séparés + 9 canaris, tests discriminants (faux entitlement après refus, patch simulé, contrat 130). Les 7 metered (natal, synastry, guide, horoscopes, lucky-days, planner) gardent moteur/corpus embarqués — documentés contournables ; transits/retrogrades = `public_content` assumé. Voir runbook premium-server-enforcement |
| **JUNO-07** | Web / API | `/api/contact` : relais mail non authentifié, sans limite de débit ni captcha | **Moyenne** | Haute | Confirmé — **FERMÉ le 18 sep 2026** : accusé public supprimé, Turnstile fail-closed + double limite durable, catégories canoniques ; fumées production EN et FR réussies (un seul courriel interne, zéro accusé, `replyTo` correct — voir runbook contact-hardening §10) |
...
| **JUNO-13** | Web | CSP sans nonce, `frame-ancestors` absent, pas de COOP/CORP/COEP | **Moyenne** | Haute | Confirmé — **EN OBSERVATION REPORT-ONLY le 21 sep 2026 (worktree)** : porte préalable PoC sur Next 15.5.25 exact (nonce ⇒ rendu par requête ; middleware seul ne démolit pas le statique ; SRI ne couvre pas le Flight inline) → politique SÉPARÉE : marketing statique (enforcement conservé + **frame-ancestors 'none', worker-src 'self', COOP same-origin, CORP same-origin** ; COEP différé documenté) ; sous-arbre /app rendu par requête (headers() au layout — force-dynamic seul mesuré inefficace sur pages client) + **Report-Only à nonce frais par requête** (strict-dynamic, zéro unsafe-inline dans les directives script, trio Turnstile conservé) ; runtime prouvé (nonces distincts, 34 scripts noncés, no-store) ; phase 2 = bascule d'enforcement après fenêtre d'observation console — voir runbook web-session-csp |
...
| **JUNO-17** | Android | `allowBackup="true"`, pas de `dataExtractionRules`, pas de Network Security Config | **Moyenne** | Moyenne | Confirmé — **PROUVÉ DANS L'AAB le 18 sep 2026 — publication non effectuée** : plugin versionné `withAndroidSecurityConfig` (5 attributs + 3 XML : exclusions cloud-backup **et** device-transfer ×5 domaines ×2 générations, cleartext interdit, CA système seules, zéro pinning, debug borné à Metro) ; prebuild prouvé idempotent ; 16 tests + validateur CI discriminant ; build EAS `juno17-proof` (autoIncrement=false — **versionCode distant 130 intact**, aucun submit) au SHA `6a7283e` → **manifeste Release fusionné : les 5 protections ✅, ressources compilées vérifiées élément par élément (aapt2)** ; `FOREGROUND_SERVICE_MEDIA_PLAYBACK` confirmé absent du Release ; restent tests fonctionnels + publication Play — voir runbook android-security-config |
| **JUNO-18** | CI/CD | Workflow sans bloc `permissions`, actions épinglées par tag, aucun scan de dépendances ni de secrets | **Faible** | Haute | Confirmé — **FERMÉ le 19 sep 2026** : PR #45 fusionnée (`4b349d7`, autorisation explicite malgré un check Vercel dupliqué mort — nettoyage Vercel séparé) ; 4 actions épinglées par SHA complet + version en commentaire ; checkout sans credentials persistants ; audit runtime WEB en CI à baseline versionnée bornée (échoue sur advisory nouveau/périmé/élargi) ; Gitleaks officiel (fetch-depth 0, config versionnée à allowlists chirurgicales, historique propre, canaris détectés) ; Dependabot npm+github-actions **reconnu — les deux écosystèmes opèrent** (zéro auto-merge) ; **CodeQL actif et publiant** (analyses sur master et PR Dependabot) ; validateur `validate:ci-security` en CI, 11 injections discriminantes ; trois workflows verts sur master (QG 1m02s, Gitleaks 16s, CodeQL 1m52s) ; **première alerte CodeQL clôturée structurellement le 21 sept** (sanitizeText mobile : la boucle v1 re-signalée à raison → **politique texte brut sans chevrons**, suppression caractère par caractère, zéro regex de sanitization — PR #62) — voir runbook ci-security |
...
| **Confiance** | Haute |
| **Statut** | **Confirmé — PARTIELLEMENT CORRIGÉ le 23 sep 2026 (reprise)** : 2/11 réellement protégées serveur (tarot devenu artefact serveur), 7/11 metered-serveur mais contournables dans l'APK (moteur/corpus embarqués), 2/11 contenu public documenté ; lissage RevenueCat local purgé au profit d'une sync vérifiée par le serveur. Migration + déploiement des edges + preuve production requis. Voir runbook premium-server-enforcement |
...
| # | Action | Effort |
|---|---|---|
| 16 | **JUNO-06 (suite)** — la reprise du 23 sep protège réellement 2/11 (tarot artefact serveur) et purge le lissage local ; restent à décider/brancher : horoscopes sur le corpus `send-daily-horoscope` (les sortirs du bundle), corpus coach derrière une edge, éphéméride serveur pour natal/synastry — une par une, hors build de publication, en mettant à jour `enforcement_class` à chaque passage | 2-4 j + suivi |
