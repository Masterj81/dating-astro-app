...
| **JUNO-06** | Mobile | 9 fonctionnalités premium sur 11 gatées côté client uniquement | **Haute** | Haute | Confirmé — **CORRIGÉ LOCALEMENT le 22 sep 2026, MIGRATION ET PREUVE PRODUCTION REQUISES** : bypass reproduits (entitlement device ouvrant les 9 features avec zéro décision serveur ; essai client-compté sans jugement ; lecture tarot complète hors réseau) puis inversés ; carte `SERVER_ENFORCED_FEATURES` exhaustive 11/11 (synastry via ses RPC existants, tarot sur les clés scindées) ; chemin legacy et helpers supprimés (gate fail-closed sur clé non mappée) ; `PremiumContext.canAccessFeature` rétrogradé en lissage abonné-transitoire documenté (ne peut rien accorder à un compte free) ; migration `20260922000001` (politiques + previews 1/jour, auto-vérifiée, **non appliquée**) ; validateurs premium-gating 11/11 + nouveau premium-data-sources (6 canaris) câblés CI. Voir runbook premium-server-enforcement |
