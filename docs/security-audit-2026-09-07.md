...
| **JUNO-18** | CI/CD | Workflow sans bloc `permissions`, actions épinglées par tag, aucun scan de dépendances ni de secrets | **Faible** | Haute | Confirmé — **corrigé LOCALEMENT le 18 sep 2026, preuve GitHub requise** : permissions déjà posées (vague 2) + checkout sans credentials persistants ; 4 actions épinglées par SHA complet 40 + version en commentaire (checkout v4.4.0, setup-node v4.4.0, gitleaks-action v3.0.0, codeql-action v3.38.1) ; audit runtime WEB en CI à baseline versionnée bornée (postcss épinglé par next, échéance 2026-12-31 — échoue sur advisory nouveau/périmé/élargi) ; Gitleaks officiel (workflow dédié, fetch-depth 0, config versionnée à allowlists chirurgicales, historique 559 commits scanné propre, canaris PEM/Stripe détectés) ; Dependabot npm+github-actions sans auto-merge ; CodeQL advanced setup périmètre versionné (default setup vérifié not-configured) ; validateur `validate:ci-security` câblé CI, prouvé par 11 injections. Restent : CI GitHub verte sur les 3 workflows, Dependabot reconnu, premier run CodeQL — voir runbook ci-security |
